...
export default function AdminModerationClient({ initialSalons = [], adminName = 'Quentin' }: AdminModerationClientProps) {
  ...
  // Actions d'approbation et de rejet avec transition
  function handleApprove(salonId: string) {
    startTransition(async () => {
      try {
        ...
        console.error(err)
        alert('Erreur lors de la validation du salon.')
      }
    })
  }
  ...
  return (
    <div className="flex min-h-screen bg-white text-slate-800 font-sans">
      ...
      <aside className="w-[80px] bg-[#04082e] text-white flex flex-col justify-between items-center py-6 flex-shrink-0 z-20 sticky top-0 h-screen overflow-hidden select-none">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Main Content Body (Exact Figma Dev Mode spacings: PL: 61px, PR: 54px) */}
        <main className="flex-1 pl-[61px] pr-[54px] py-10 flex flex-col gap-8 w-full">
          
          {/* Page Header (Title + Subtitle) */}
          <div>
            <h1 className="font-heading font-bold text-[32px] leading-tight text-[#04082E] tracking-tight">
              Modération des Salons
            </h1>
            <p className="font-sans text-[15px] text-[#6E6E6E] mt-1 font-normal">
              Vérification et validation des dossiers professionnels
            </p>
          </div>

          {/* Inline Tabs and Search Bar (100% Figma layout) */}
          <div className="flex flex-row items-end justify-between border-b border-slate-200 w-full pb-0.5">
            ...
            {/* Inline search bar (Figma bottom line style) */}
            <div className="pb-2.5 flex items-center gap-2 border-b border-black w-60">
          ...
          {/* Figma Styled DataTable Card */}
          <div className="border border-[#E2E8F0] rounded-[8px] bg-white shadow-sm overflow-hidden flex flex-col justify-between">
                  ...
                  {paginatedSalons.map((salon) => {
                    return (
                      ...
                      >
                        ...
                        {/* Column: JUSTIFICATIF (Figma blue pill) */}
                        <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                          {salon.url_justificatif_local ? (
                            <a
                              ...
                              className="inline-flex items-center px-4 py-1.5 bg-[#E2EAFE] hover:bg-blue-100 text-[#0738dc] rounded-[4px] text-xs font-bold transition shadow-sm"
                            >
                            ...
                            {salon.statut_validation === 'waiting' ? (
                              <>
                                <button
                                  ...
                                  disabled={isPending}
                                  className="px-5 py-2 bg-[#0738dc] hover:bg-blue-700 text-white rounded-[4px] text-xs font-medium shadow-sm transition active:scale-95 flex items-center justify-center"
                                >
                                  Valider
                                </button>
                                <button
                                  ...
                                  disabled={isPending}
                                  className="px-5 py-2 border border-[#EF4444] text-[#EF4444] hover:bg-red-50 rounded-[4px] text-xs font-medium transition active:scale-95 flex items-center justify-center"
                                >
                                  Rejeter
                                </button>
                              </>
                              ...
                              <button 
                                onClick={() => setSelectedSalon(salon)}
                                className={`px-4 py-1.5 rounded-[4px] text-xs font-semibold border transition
                                ...
                                `}
                              >
                                {salon.statut_validation === 'approved' ? 'Validé' : 'Rejeté'} (Fiche)
                              </button>
                            )}
                          </div>
                        </td>
            ...
            <div className="bg-white px-6 py-4 border-t border-[#E2E8F0] flex items-center justify-between text-xs text-slate-500 font-semibold">
              <span className="font-sans font-normal text-[#6E6E6E]">
                {filteredSalons.length} salons en attente - Page {currentPage} sur {totalPages}
              </span>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
                  disabled={currentPage === 1}
                  className="w-8 h-8 flex items-center justify-center border border-[#CCCCCC] hover:bg-slate-50 text-[#0738dc] rounded-[4px] font-bold text-xs disabled:opacity-40 disabled:cursor-not-allowed transition select-none"
                >
                  &lt;
                </button>
                <button
